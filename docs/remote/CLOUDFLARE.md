...
- Dashboard status and command endpoints are intentionally tokenless for this short-lived remote-control setup.
...
- The agent endpoints remain protected by `AGENT_TOKEN` so an unrelated client cannot impersonate the home-PC agent.
...
- `AGENT_TOKEN`: stored only in Cloudflare and on the home PC; authorizes `/api/agent/*`.
...
The same value can be created in the Cloudflare dashboard instead.

## Home PC setup

After the Worker is deployed and its runtime secret exists, set:
...
Open the Worker URL on the teacher iPad. No dashboard token is required. The dashboard refreshes status every two seconds. Commands are delivered through the home agent's held Cloudflare request, so command start does not wait for the dashboard refresh interval.
