...
class NativeReachabilityDifferentialTests(unittest.TestCase):
    ...
    def test_flat_rotation_traversal_matches_legacy_reference(self) -> None:
        native = reachability_native_module._native
        self.assertIsNotNone(native)
        reference_run = native.run_packed_rotation_reference
        ...
        for case in range(48):
            ...
            with self.subTest(case=case, piece=piece, allow_180=allow_180):
                self.assertEqual(flat["placementsPacked"], legacy["placementsPacked"])
                self.assertEqual(flat["placementCount"], legacy["placementCount"])
                self.assertEqual(flat["counters"], legacy["counters"])
