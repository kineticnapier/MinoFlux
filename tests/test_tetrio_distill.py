...
from minoflux_ai import CaptureSample, align_capture_samples, reachable_placements
...
class TetrioDistillTests(unittest.TestCase):
    def test_capture_and_alignment_write_neural_ranking_samples(self) -> None:
        ...
        alignments = align_capture_samples(samples, allow_180=True)
        self.assertTrue(all(item.status in ("exact", "ambiguous") for item in alignments))
    ...
    def test_used_hold_sample_is_skipped_in_first_distillation_pass(self) -> None:
        ...
        alignment = align_capture_samples((sample,), allow_180=True)
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            capture_path = root / "capture.jsonl"
            alignment_path = root / "alignment.jsonl"
            ...
            alignment_path.write_text(json.dumps(alignment[0].to_dict()) + "\n", encoding="utf-8")
