...
def solo():
    seeds,pieces=((74011,74047),90) if PHASE=='short' else ((94013,94061,94109),150)
...
def versus_game(seed: int, swapped: bool):
    match=VersusMatch(seed,garbage_cap=8); turn='player'; turns=0; max_b2b={'player':0,'ai':0}; candidate_side='ai' if swapped else 'player'
    ...
    c=match.side(candidate_side); other='player' if candidate_side=='ai' else 'ai'; b=match.side(other); winner=match.winner or 'draw'
    return {'candidate_win':int(winner==candidate_side),'baseline_win':int(winner not in ('draw',candidate_side)),'draw':int(winner=='draw'),'candidate_attack':c.game.attack,'baseline_attack':b.game.attack,'candidate_sent':c.sent,'baseline_sent':b.sent,'candidate_canceled':c.canceled,'baseline_canceled':b.canceled,'candidate_received':c.received,'baseline_received':b.received,'candidate_max_b2b':max_b2b[candidate_side],'baseline_max_b2b':max_b2b[other],'candidate_topout':int(c.game.game_over),'baseline_topout':int(b.game.game_over)}

def versus():
    rows=[versus_game(140017+(i//2)*53,bool(i%2)) for i in range(6)]; n=len(rows); avg=lambda k: sum(r[k] for r in rows)/n
