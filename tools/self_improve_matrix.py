...
def patched_context(game, features, weights):
    ...
    return base if ACTIVE_NAME == "baseline" else base + extra_context_score(ACTIVE_NAME, game, features)


def _versus_sides(match, root_side):
    if root_side == "player":
        return match.player, match.ai
    return match.ai, match.player


def patched_versus_score(match, root_side, **kwargs):
    base = ORIGINAL_VERSUS_SCORE(match, root_side, **kwargs)
    if ACTIVE_NAME not in VERSUS_ONLY or abs(base) >= 900_000:
        return base
    ...
    if ACTIVE_NAME == "pending_height_emergency":
        ...
        if resolution.garbage_applied:
            ...
            extra -= direction * 0.24 * resolution.garbage_applied * (
                1.0 + burden.holes + burden.max_height / 8.0
            )
    return base + extra
...
def solo_summary(name: str, stage: str):
    ...
    return {
        ...
        "meanBumpiness": result.mean_bumpiness,
...
def versus_summary(name: str):
    ...
    return {
        ...
        "canceled": [
            sum(g.player_canceled for g in result.per_game) / result.games,
            sum(g.ai_canceled for g in result.per_game) / result.games,
        ],
        "received": [
            sum(g.player_received for g in result.per_game) / result.games,
            sum(g.ai_received for g in result.per_game) / result.games,
        ],
        "maxB2B": [
            sum(g.player_max_b2b for g in result.per_game) / result.games,
            sum(g.ai_max_b2b for g in result.per_game) / result.games,
        ],
        "topouts": [
            sum(g.winner == "ai" for g in result.per_game),
            sum(g.winner == "player" for g in result.per_game),
        ],
    }
